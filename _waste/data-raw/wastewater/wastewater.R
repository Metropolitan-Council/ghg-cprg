...
wastewater_out <- county_wastewater %>% 
  mutate(co2e = if_else(substr(NAME,nchar(NAME)- 8,nchar(NAME)) == "Wisconsin",epa_co2e ,state_co2e),
         source = if_else(substr(NAME,nchar(NAME)- 8,nchar(NAME)) == "Wisconsin","Environmental Protection Agency" ,"Minnesota Pollution Control Agency")) %>% 
...
wastewater_meta <-
  tibble::tribble(
    ...
    "co2e", class(wastewater_out$co2e), "Metric tonnes of CO2 equivalency generated from municipal wastewater",
