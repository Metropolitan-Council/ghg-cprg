...
The previous iteration of this inventory calculated solid waste emissions by multiplying activity totals by emissions factors from the EPA's Emissions Factor Hub. This update instead uses methodologies recommended by the Intergovernmental Panel on Climate Change.
...
The methane commitment model calculates methane emissions from landfills for a given year by multiplying municipal solid waste totals by a methane generation potential and adjusting for methane recovery and oxidation,

$$Emissions_{CH_4} = [(MSW \times L_0 ) – rec] \times (1 – OX) $$

where MSW is the municipal solid waste disposed of in landfills (in metric tons), from MPCA SCORE data; rec is the amount of methane recovered by flaring or landfill gas to energy programs, also in metric tons, from the EPA's State Inventory Tools, and OX is the oxidation factor, assumed to be 0.1 in line with IPCC default values. \[REWRITE FOR CLARITY AND EXPLANATION OF RECOVERY CALCULATIONS\]

$L_0$ is a methane generation potential calculated as follows:
...
The emissions factors of 10 and 0.6 come from IPCC default values.
...
Incineration of waste produces CH~4~, CO~2~, and N~2~O emissions. However, the Global Protocol for Community-Scale Greenhouse Gas Emission Inventories reports negligible CH~4~ emissions for continuous incineration facilities. [CHECK IF WE USE STOKERS OR FLUIDIZED BEDS - STOKER EF ~0.2]
...
-   $EF_{N_2O}$ = aggregate N~2~O emission factor for MSW. Assigned GHG Protocol default of 50 g N~2~O/ tonnes waste for continuous and semi-continuous incinerators.

### Misc

note lack of industry data
