...
metc_solidwaste_inventory <- county_emissions %>%
  ...
  mutate(
    ...
    inventory_year = as.character(year),
    geoid = case_when(
      geog_name %in% c("Pierce", "St. Croix") ~ paste0("55", geog_id),
      .default = paste0("27", geog_id)
    ),
    value_emissions = emissions_metric_tons_co2e,
    units_emissions = "Tonnes CO2e"
  ) %>%
  select(
    names(epa_waste_inventory)
  )
