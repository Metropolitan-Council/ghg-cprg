...
```{r fig-transportation-county-emissions-by-source}
#| fig-cap: `{r} paste0(transportation_current_year, " annual emissions by vehicle weight")`
#| out-width: "95%"

fig_transportation_county_emissions_by_fuel <- plot_ly(
  type = "bar",
  source = "fig-transportation-county-emissions-by-fuel",
  data = onroad_emissions %>% 
    filter(emissions_year == transportation_current_year),
  y = ~reorder(county_name, emissions_metric_tons_co2e),
  x = ~emissions_metric_tons_co2e,
  ...
  plotly_layout(
    main_title = paste0(transportation_current_year, " annual emissions by fuel type"),
    subtitle = "",
    y_title = "County",
    x_title = "Metric tons CO<sub>2</sub>e",
