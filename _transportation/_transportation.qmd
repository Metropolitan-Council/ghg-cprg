...
plot_ly(
  ...
  add_trace(data = nei_emissions_all,
...
We used the LGGIT Excel workbook tool with the following inputs in the "Mobile-Data" sheet

```{r}
lggit_vmt_entries <- readRDS(file.path(here::here(), "_transportation/data-raw/epa/lggit_vmt_entries.RDS"))

kable(lggit_vmt_entries)
```
...
(lggit_totals$total %>% sum()) %>% scales::comma()
```

We can test this by looking at the emissions factors for LGGIT and compare them with the EPA MOVES4 values
