## State DOT data {#sec-state-dot-data}
...
Due to limitations in data consistency, the MnDOT CTU dataset was subsetted to only include CTUs with reliable data. Reliable CTUs are defined as such:

- CTU has a complete time series from 2014-`r max(mndot_ctu_summary$vmt_year)` with no missing years. 2015 data were interpolated in the same manner as the county VMT data. 
- CTU has sampled data on local route systems (including Municipal State Aid Streets) during any year from 2017-`r max(mndot_ctu_summary$vmt_year)`. Read more about route system designations in [mndot_route_system.R](data-raw/mndot_route_system.R).

Based on collaboration with regional partners and MnDOT communications, we assigned all non-municipal route system miles for Ramsey County to White Bear Township starting in 2006.

Only `r unique(mndot_vmt_ctu$ctu_name) %>% length()` cities were considered reliable for analysis. All other CTUs were modeled. See @sec-gap-fill-ctu-vmt for more information.
...
mndot_ctu_summary %>%
  ...
  plotly_layout(
    ...
    x_title = "Year",
    y_title = "Annual VMT",
    legend_title = "Top 10 CTUs by VMT",
