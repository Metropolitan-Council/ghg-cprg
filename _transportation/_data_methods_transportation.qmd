...
tbl_inv_forecast_sources <- county_inventory_forecast_emissions %>% 
  ...
  mutate(Years  = case_when(Years == "2050 - 2050"  ~ "2050",
                            TRUE ~ Years)) %>% 
  arrange(Years)
...
The Met Council also completed city-level VMT estimates. 
...
tbl_ctu_modeled_vmt <- mndot_vmt_ctu_gap_filled %>% 
  ...
  mutate(Years  = case_when(Years == "2050 - 2050"  ~ "2050",
                            TRUE ~ Years),
         Cities = case_when(final_vmt_source == "Regional Travel Demand Model" ~ "All",
                            TRUE ~ Cities)) 
...
All data processing for transportation is compiled using [_run_all_transportation.R](data-raw/_run_all_transportation.R), ensuring all disparate datasets and sequential processing requirement are correct.  
