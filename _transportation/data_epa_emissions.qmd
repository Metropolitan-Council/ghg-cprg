## EPA emissions data {#sec-epa-emissions-data-transportation}
```{r}
...
epa_onroad_source_set %>%
  ...
  summarize(
    emissions_years = paste0(min(emissions_year), " - ", max(emissions_year)),
    .groups = "keep"
  ) %>%
  mutate(emissions_years = case_when(
    emissions_years == "2022 - 2022" ~ "2022",
    emissions_years == "2021 - 2021" ~ "2021",
    emissions_years == "2020 - 2020" ~ "2020",
    TRUE ~ emissions_years
  )) %>%
  ungroup() %>%
...
```{r tbl-epa-vehicle-types}
#| tbl-cap: "Vehicle types provided by EPA datasets"
#| out-width: "95%"

onroad_emissions %>%
  select(vehicle_weight_label, vehicle_types_included) %>%
  unique() %>%
  group_by(vehicle_weight_label) %>%
  filter(nchar(vehicle_types_included) == max(nchar(vehicle_types_included))) %>%
  ungroup() %>%
...
```{r tbl-epa-fuel-types}
#| tbl-cap: "Vehicle fuel types provided by EPA datasets"
#| out-width: "95%"

onroad_emissions %>%
  ungroup() %>%
  select(vehicle_weight_label, fuel_types_included) %>%
  unique() %>%
  tidyr::separate_wider_delim(fuel_types_included,
    delim = ", ",
    names_sep = "_",
    too_few = "align_start"
  ) %>%
  ...
  select(-name) %>%
  group_by(vehicle_weight_label) %>%
  summarize(fuel_types_included = paste0(sort(unique(value)), collapse = ", ")) %>%
  ungroup() %>%
  arrange(vehicle_weight_label) %>%
  select(
    `Vehicle weight label` = vehicle_weight_label,
...
### Consistent EPA data limitations {#sec-epa-emissions-limitations}
...
- The 2020 NEI had challenges in data availability due to COVID-19 pandemic effects on transportation behavior. 
...
- Though nitrous oxide  N~2~O has a high global warming potential (@sec-gwp), the amount of N~2~O released is relatively small when compared to other sectors. N~2~O is unavailable in EQUATES, except years 2018 and 2019. 
