...
trip_length_bin <- county21_data$od_trip_all %>%
  ...
  rowwise() %>%
  mutate(length_bin = stringr::str_remove(name, "trip_length_") %>%
...
#| fig-cap: "StreetLight trip length distribution widget"
#| out-width: "95%" 
...
bscols(
  ...
  list(
    ...
    filter_select(id = "origin_zone_name", 
                  label = "Origin county",
                  sharedData = shared_trip_length_bin,
                  ~origin_zone_name,
                  multiple = FALSE),
    filter_select(id = "destination_zone_name", 
  ...
  plot_ly(
    ...
    layout(yaxis = list(format = "%"))
)
```

```{js}
function remove_all_option() {
    document.getElementById("origin_zone_name")[0].selectize.removeOption("");
    document.getElementById("destination_zone_name")[0].selectize.removeOption("");

}
window.onload = remove_all_option;

function filter_default() {
    document.getElementById("origin_zone_name").getElementsByClassName("selectized") 
[0].selectize.setValue("Anoka", false);

document.getElementById("destination_zone_name").getElementsByClassName("selectized") 
[0].selectize.setValue("Anoka", false);
 }
