...
stl_zone_volume <- county21_data$zone_od_all %>% 
  filter(
    day_type == "0: All Days (M-Su)",
    day_part == "0: All Day (12am-12am)"
  ) %>%
  group_by(zone_name) %>% 
  summarize(average_daily_zone_traffic_st_l_volume = sum(average_daily_zone_traffic_st_l_volume))



hpms_streetlight <- left_join(
  stl_zone_volume,
  hpms_vmt,
  by = c("zone_name" = "county")
) %>% 
  unique()
```
...
```{r}
plot_ly(
  data = hpms_streetlight,
  x = ~daily_vmt,
  y = ~average_daily_zone_traffic_st_l_volume,
  type = "scatter",
  mode = "markers",
  hoverinfo = "text",
  # hovertemplate = passenger_dist_area$hovertext,
...
council_leaflet() %>% 
  addCircleMarkers(data = commercial_points,
                   popup = commercial_points$hover_info) %>% 
  addPolylines(data = commercial_lines,
               popup = commercial_lines$hover_info)
```


```{r}
calibration_long <- commercial_calibration_points %>% 
  ...
  sf::st_drop_geometry() %>% 
  tidyr::pivot_longer(cols = c(personal_traffic_ratio, 
                               medium_commercial_ratio,
                               heavy_commercial_ratio),
                      names_to = "vehicle_weight") %>% 
  mutate(vehicle_weight = 
           case_when(vehicle_weight == "personal_traffic_ratio" ~ "Passenger",
...
MnDOT provides AADT road segments, which align with station identification numbers. Wisconsin does not readily supply road segment data - as suggested by the Wisconsin cartographers office [@statecartographersofficeWisconsinRoadData2021], we pulled OpenStreetMaps road data. 
