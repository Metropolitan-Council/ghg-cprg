...
stl_vmt <- readRDS(file.path(here::here(), "_transportation/data/county_vmt_emissions.RDS"))
...
stl_vmt_all <-  stl_vmt %>% 
  group_by(zone) %>% 
  summarise(vmt_total = sum(vmt_total),
...
hpms_streetlight <- left_join(
  ...
  unique() %>% 
    mutate(hovertext = paste0(
    "County: ", zone, "<br>",
    "HPMS AADT: ", round(annual_vmt*1e-9), "B<br>",
    "StreetLight VMT: ", round(vmt_total*1e-9), "B<br>",
    "<extra></extra>"
  ))
...
plot_ly(
  ...
  mode = "markers",
  hoverinfo = "text",
  hovertemplate = hpms_streetlight$hovertext,
  opacity = 0.7,
  marker = list(
    size = 18,
    line = list(
