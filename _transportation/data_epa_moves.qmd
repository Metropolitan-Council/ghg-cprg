...
The resulting table provides grams of CO~2~, CH~4~, and N~2~O per vehicle mile traveled. We imported, processed, and saved the data in an R script, [epa_moves.R](_transportation/data-raw/epa_moves.R). CO~2~ equivalence (CO~2~e) values are derived using global warming potential (GWP) values. See @sec-gwp for more details.
...
epa_moves_wide <- epa_plus_moves %>% 
  select(-ghg_factor, -`Vehicle model year`, 
