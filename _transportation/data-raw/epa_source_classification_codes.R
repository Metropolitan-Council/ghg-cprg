...
scc_combine <- scc_complete_road %>%
  ...
  mutate(
    vehicle_fuel_label =
      case_when(
        str_detect(fuel_type, "non-Diesel") ~ "Non-Diesel",
        str_detect(fuel_type, "Diesel") ~ "Diesel",
        str_detect(fuel_type, "Gasoline") ~ "Gasoline",
        TRUE ~ "Other"
      ),
    vehicle_weight_label = case_when(
      vehicle_type %in% c("Trucks and buses",
                          "Light commercial trucks",
                          "Single unit short-haul trucks",
                          "Single unit long-haul trucks"
      ) ~ "Medium-duty",
      vehicle_type %in% c("Combination long-haul trucks",
                          "Combination short-haul trucks",
                          "Refuse trucks") ~ "Heavy-duty",
      vehicle_type %in% c("Intercity buses",
                          "Transit buses",
                          "School buses") ~ "Transit",
      vehicle_type %in% c("Passenger cars",
                          "Passenger trucks",
                          "Motorcycles") ~ "Passenger",
      vehicle_type %in% c("Motor homes",
                          "Gas stations",
