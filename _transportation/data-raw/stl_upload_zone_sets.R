...
stl_counties <- cprg_counties %>% 
  mutate(zone_name = NAME,
         is_pass = 1) %>% 
  select(zone_name, is_pass)
...
# create zone name, passthrough = TRUE
stl_ctu <- cprg_ctu %>% 
  mutate(zone_name = paste0(CTU_NAME, "_", CTU_CLASS, "_", COUNTY_NAM),
         is_pass = 1) %>% 
  select(zone_name, is_pass)
