...
freight_calibration_lines <- bind_rows(mn_stations_lines,
                                       wi_stations_lines) %>% 
  mutate(is_pass = 1,
         is_bidi = 1,
  ...
  rowwise() %>% 
  mutate(total = sum(personal_traffic_ratio,
                     medium_commercial_ratio,
                     heavy_commercial_ratio)) %>% 
  mutate(personal_traffic_ratio = case_when(total == 1 ~ personal_traffic_ratio,
                                            total > 1 ~ personal_traffic_ratio - 0.01,
                                            total < 1 ~ personal_traffic_ratio +  0.01)) %>% 
  mutate(total = sum(personal_traffic_ratio,
                     medium_commercial_ratio,
                     heavy_commercial_ratio)) %>% 
  mutate(medium_commercial_ratio = case_when(total == 1 ~ medium_commercial_ratio,
...
to_degrees <- function(rad){((rad * 180) / (pi)) %% 359}
