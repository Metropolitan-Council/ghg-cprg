...
read_airemis_modeling <- function(equates_path) {
  emis_mod_table <- data.table::fread(
    equates_path,
    skip = 19,
    header = FALSE,
  ...
  saveRDS(emis_mod_table, 
          file.path(
            paste0("_transportation/data-raw/epa/air_emissions_modeling/air_emissions_modeling_mn_wi/onroad/", 
                   out_file_name)))
...
# combine and save 
onroad_mn_wi <- purrr::map(list.files("_transportation/data-raw/epa/air_emissions_modeling/air_emissions_modeling_mn_wi/onroad/", full.names = TRUE),
                           readRDS) %>% 
  bind_rows()
