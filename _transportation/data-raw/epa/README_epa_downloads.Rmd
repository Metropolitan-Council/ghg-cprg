...
epa_onroad_source_set %>% 
  arrange(data_source, emissions_year) %>% 
  gt() %>% 
  gt::opt_row_striping() %>% 
  tab_style(
...
```{r tbl-nei-locations, tab.cap="NEI download direct URLs"}
epa_downloads %>% 
  filter(group == "NEI",
         measurement %in% c("emissions",
                            "smoke_flat_file")) %>% 
...
EQUATES only has onroad data from 2002 to 2017.

Within /air_emissions_modeling/EQUATES, create a folder for each EQUATES year (2002, 2005, 2008, 2011, 2014, 2017) with the prefix "EQUATES_", plus one additional folder named "EQUATES_MN_WI".

Download the data for each of the given links and save in the appropriate location. 

EQUATES is processed in [epa_equates_read.R](./../epa_equates_read.R)
