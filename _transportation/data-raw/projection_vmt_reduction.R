...
pop_reduction <- co_pop %>% 
  left_join(co_vmt, 
            by = join_by(geoid, county_name, cprg_area,
  ...
  mutate(vmt_per_person = annual_vmt/population,
...
vmt_per_person_forecast <- 
  ...
  filter(cprg_area == TRUE) %>% 
  group_by(vmt_year) %>% 
  summarise(network_passenger_annual = sum(network_passenger_annual),
  ...
  left_join(
    demographic_forecast_11_county %>% 
      ...
      mutate(inventory_year = case_when(inventory_year == 2022 ~ 2023,
                                        TRUE ~ inventory_year)),
    by = c("inventory_year")) %>% 
  mutate(vmt_per_person = network_vmt/value,
