...
epa_equates_summary_vehicle <- epa_equates_summary_interp %>%
  group_by(emissions_year, geoid, county_name,
...
equates_diff <- equates_cmas_summary %>% 
  select(emissions_year, geoid, county_name, data_source,scc6_desc, scc6, fuel_type, vehicle_type,
         co2,
         emissions_metric_tons_co2e) %>% 
  full_join(epa_equates_summary_interp %>% 
              select(emissions_year, geoid, county_name, data_source,scc6_desc,
                     scc6, fuel_type, vehicle_type,
                     interpolation,
                     co2,
                     emissions_metric_tons_co2e),
            by = join_by(emissions_year, geoid, county_name, data_source, 
                         scc6_desc,
                         scc6, fuel_type, vehicle_type),
            suffix = c(".cmas", ".metc")
  ) %>% 
...
veh_type_comp <- bind_rows(
  ...
  pivot_wider(names_from = download_source,
...
bind_rows(
  ...
  filter(county_name == "Hennepin") %>% View
