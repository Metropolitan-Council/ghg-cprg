...
nonroad_correct <- nei_nonroad_emissions %>%
  ...
  left_join(scc_sector) %>%
  select(
    geoid, state_name, county_name, scc, scc_new, scc6, scc6_new, nei_inventory_year,
    total_emissions, pollutant_code, emissions_uom,
    pollutant_desc, data_set
  ) %>%
  mutate(
    scc = ifelse(scc != scc_new, scc_new, scc),
...
nei_scc_emissions <- bind_rows(
  nei_onroad_emissions %>%
...
nei_emissions_summary <- nei_scc_emissions %>%
  group_by(
    geoid, county_name, cprg_area, nei_inventory_year, data_category,
