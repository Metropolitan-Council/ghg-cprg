...
calculate_vmt <- function(data_list, class = "passenger") {
  ...
  od_same <- od_trips %>%
    ...
    dplyr::mutate(
      ...
      vmt_same = vmt_year
    ) %>% # create new variable with the vmt for same origin destination trips
    dplyr::select(
      analysis_name, mode_of_travel, zone,
      vehicle_weight, vmt_same
    ) %>% # select only the zone and vmt columns
