...
wis_station_ratios <- wis_stations %>% 
  filter(COUNTY %in% c("St. Croix",
                       "Pierce")) %>% 
  select(TRAF_SITEID, LOCATION, COUNTY, YEAR, MAX_YEAR,
         DURATION,
  ...
  filter(MAX_YEAR >= 2017,
         DURATION == "Continuous",
         YEAR == MAX_YEAR) %>% 
  mutate(passenger = LIGHT_DUTY_VEHICLES/100,
...
wis_roads_nearest <- wis_roads[sf::st_nearest_feature(wis_station_locations,
                                                      wis_roads),] %>% 
