...
baseline_emissions <- readRDS(file.path(here::here(), "_meta/data/cprg_county_emissions.RDS"))
...
nei_small <- left_join(nei_emissions %>% 
  ...
  left_join(baseline_emissions %>% 
              filter(year %in% c(2017,2020)s))
  mutate(value_emissions_double = if_else(is.na(value_emissions_double),0,value_emissions_double)) %>% 
  mutate(value_emissions = as.numeric(values_emissions) - value_emissions_double,
         data_source = "NEI - all") %>% 
...
## Introduction
Industrial facilities are major emitters stemming from high electricity demand, on-site fuel combusion, and industrial processes.
