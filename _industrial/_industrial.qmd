...
industrial_gas <- readRDS(file.path(here::here(), "_industrial/data/industrial_emissions_by_gas.RDS"))
...
ind_emissions_2022 <- county_emissions %>%
  filter(emissions_year == 2022, sector == "Industrial",
         !category %in% c("Building Fuel", "Electricity")) %>%
  pull(value_emissions) %>%
  sum()

ind_co2_2022 <-  industrial_gas %>% 
  filter(inventory_year == 2022,
         units_emissions == "Metric tons CO2") %>% 
  pull(metric_tons_co2e) %>% 
  sum()

refinery_emissions_2022 <- ghgrp_emissions %>%
  filter(inventory_year == 2022, primary_naics_code == 324110) %>%
  pull(value_emissions) %>%
  sum()
...
fig_ind_county_emissions <- plot_ly(
  ...
  plotly_layout(
    main_title = "2022 county industrial emissions",
    x_title = "Million metric tons CO<sub>2</sub>e (log scale)",
    subtitle = ""
  ) %>%
  layout(
    barmode = "stack",
    ...
    hoverlabel = list(
      font = list(
        ...
        xaxis = list(
      type = "log",
      title = "Million metric tons CO<sub>2</sub>e (log scale)"
    )
  )
...
Industrial emissions are varied, including carbon dioxide, methane, nitrous oxide, and fluorinated gases from industrial processes. These greenhouse gases range from 28 to 24,300% more potent in trapping heat compared to carbon dioxide and are displayed below in their carbon dioxide equivalencies to make their effect more comparable. Despite being less potent, carbon dioxide is still represents `r 100*round(ind_co2_2022 / ind_emissions_2022, digits = 3)`% of industrial emissions due to being the most common byproduct of industry.


```{r fig-ind-county-emissions-by-gas-type}
#| fig-cap: "2022 county agricultural emissions by gas type"
#| out-width: "95%"

emissions_by_gas_2022 <- industrial_gas %>% 
  filter(inventory_year == 2022) %>% 
  mutate(gas_type = str_replace_all(units_emissions, "Metric tons ",""))

fig_ind_county_emissions_by_gas_type <- plot_ly(
  type = "bar",
  source = "fig-ind-county-emissions-by-gas-type",
  data = emissions_by_gas_2022,
  y = ~ reorder(county_name, metric_tons_co2e ),
  x = ~metric_tons_co2e,
  color = ~gas_type,
  hovertemplate = ~ paste0(
    county_name, "<br>",
    gas_type, "<br>",
    round(metric_tons_co2e , digits = 2), "  metric tons of CO<sub>2</sub>e", "<br>",
    "<extra></extra>"
  )
) %>%
  plotly_layout(
    main_title = "2022 county industrial emissions by gas type",
...
As industrial emissions are provided as point sources, scaling to cities is already done. Note that the color ramp in the map below is on a log scale due to the high magnitude of emissions from two oil refineries - pop-up values are reported as untransformed values, however.


```{r fig-ctu-ind-emissions}
#| fig-cap: "2022 city level industrial emissions"
#| out-width: "95%"
#| out-height: "500px"
...
ind_emissions_ctu_2022 <- baseline_emissions %>%
  filter(inventory_year == 2022) %>%
  group_by(city_name) %>%
