...
ghgrp <- lapply(as.character(2011:2023), function(y) {
  read_excel(file.path(here::here(),
                       paste0( "_industrial/data-raw/ghgrp/ghgp_data_",
                               y,
                               ".xlsx")), 
             sheet = 1,
             skip = 3) %>% 
    ...
    mutate(unit_emissions = "Metric tons of CO2e",
           inventory_year = y)
}) %>% 
  bind_rows()
...
ghgrp_source <- ghgrp %>% 
  select(-c(14:26)) %>% 
  pivot_longer(cols = 14:50, 
               names_to = "emission_source", 
               values_to = "value_emissions",
               values_transform = list(value_emissions = as.numeric)) %>% 
  filter(!is.na(value_emissions)) %>% 
  mutate(doublecount = if_else(emission_source %in% c("electricity_generation",
         "municipal_landfills"),
         "Yes", "No"),
