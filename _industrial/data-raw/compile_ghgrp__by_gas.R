...
refinery_gas <- ghgrp %>%
  ...
  mutate(
    mt_gas = case_when(
      ...
      grepl("n2o", gas_type) ~ value_emissions / 298
    ), # they used IPCC 4th assessment values, so can't use stored values
...
ghgrp_other <- ghgrp %>%
  filter(
    !grepl("Y", industry_type_subparts), # remove refineries
    !grepl("D", industry_type_subparts), # remove power plants
    !(grepl("Waste", industry_type_sectors) & is.na(industrial_waste_landfills))
  ) %>% # remove municipal waste
  # remove facilities with only stationary combustion
  filter(
    rowSums(
      across(
        electricity_generation:industrial_waste_landfills,
        ~ as.integer(!is.na(.) & . > 0)
      )
    ) > 0
  )
...
ghgrp_gas <- ghgrp_other %>%
  ...
  filter(!is.na(value_emissions)) %>%
  mutate(
    gas_type = str_replace_all(gas_type, "_emissions", ""),
    mt_gas = case_when(
...
process_gas <- left_join(
  ...
  mutate(
    mt_gas = mt_gas.x - coalesce(mt_gas.y, 0),
