...
eGRID_mix <- data.frame(
  ...
  Percentage = c("39.60%", "0.20%", "10.60%", "0.10%", "8.60%", "4.40%", "0.80%", "34.60%", "0.90%", "0.00%", "0.20%", "N/A")
)
...
We compared our county-level emissions figures to emissions estimates derived from the EIA's State Electricity Profiles, which were generated by down-scaling EIA's Minnesota and Wisconsin state-level numbers for total electricity retail sales. We achieved this by applying the proportions of each state's population residing in each of our study area counties to generate county-level emissions estimates. Our inventory quantified X metric tons of electricity in our study area relative to 20,214,537 metrics tons based on the aforementioned downscaled statewide retail numbers.

```{r fig-eiaQA}
#| fig-cap: "METC emissions inventory v. downscaled EIA State Electricity Profiles"
#| out-width: "95%"
...
eiaQA_all <- bind_rows(mn_eiaQA, wi_eiaQA)

EIAvMETC_tons <- eiaQA_all %>%
  mutate(downscaled_EIA_total_CO2e_emissions_tons = downscaled_EIA_total_CO2e_emissions_lbs / 2000,
         OURS_total_CO2e_emissions_tons = OURS_total_CO2e_emissions_lbs / 2000,
         geo = paste0(county_name, ", ", state)
) %>%
  pivot_longer(
  ...
  mutate(
    inventory_source = recode(inventory_source, 
                             ...
                             OURS_total_CO2e_emissions_tons = "MetC - current inventory")
  ) 
...
eiaQA_plot <- plot_ly(
  data = EIAvMETC_tons,
  x = ~reorder(geo, -emissions_metric_tons_co2e),
  y = ~emissions_metric_tons_co2e,
  source = "fig-eiaQA",
  ...
  layout(
    title = "Electricity Emissions Inventory QA<br><sub>Comparison between MetC and EIA State Electricity Profiles</sub>",
    yaxis = list(title = "Emissions (metric tons CO2e)"),
    xaxis = list(title = "County"),
    barmode = 'group',
    hovermode = 'closest',
    legend = list(
      title = list(text = 'Inventory Source'),
      font = list(size = 10),
      traceorder = 'normal' 
    )
  )
...
nrel_MetC_Comp <- nrelQA %>%
  left_join(elecEmissions_all %>% select(county,
...
nrelQA_plot <- plot_ly(
  ...
  layout(
    title = "Electricity Emissions Inventory QA<br><sub>Comparison between MetC and NREL SLOPE</sub>",
    yaxis = list(title = "Emissions (metric tons CO2e)"),
    xaxis = list(title = "County"),
    barmode = 'group',
    hovermode = 'closest',
