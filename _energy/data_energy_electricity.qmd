...
utilities <- unique(WI_inScopeUtilities$utility_name)
color_palette <- colorFactor(palette = "viridis", domain = inScopeUtilities)
...
WI_elec_servTerrs <- council_leaflet() %>%
  addPolygons(
    data = WI_elecCoop %>% sf::st_transform(4326),
    fill = TRUE,
    fillColor = ~color_palette(LABEL),  # Use the color mapping
    stroke = TRUE,
    color = "gray",
    weight = 2,
    popup = WI_elecCoop$LABEL
