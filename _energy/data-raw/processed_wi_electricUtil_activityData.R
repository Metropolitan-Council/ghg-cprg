...
combined_wi_elecUtil_activityData <- st_drop_geometry(WIutilities_in_scope_CensusBlockSums) %>%
  ...
  left_join(
    st_drop_geometry(fullElecUtilityServiceAreas_CensusBlockSums) %>%
      rename(
...
WI_currentCounty_deliveries <- read_rds(here("_energy", 
  ...
  mutate(EIA_WI_stateTotal = EIA_WI_elecRetailEst_mWh)
...
write_rds(downscaleEIA_WI_electricRetail, here("_energy", "data", "wisconsin_QA_versusEIAStateProfile.RDS"))
