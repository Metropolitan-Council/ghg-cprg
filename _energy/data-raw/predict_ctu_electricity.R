...
rf_res_train <- randomForest(mWh_delivered ~ 
                     ctu_class + #get community designation here
                     total_pop + total_households + total_residential_units +mean_year_apartment +
                     mean_year_multifamily_home + mean_year_single_family_home + 
                     total_emv_apartment + total_emv_single_family_home + total_emv_multifamily_home +
...
prediction_comparison_res <- rbind(county_res_predict %>% 
...
### can subset predict test model?
rf_nonres <- randomForest(mWh_delivered ~ 
                            total_job_spaces +
                            max_office +
                            max_commercial +
                            max_institutional +
...
prediction_comparison_nonres <- rbind(county_nonres_predict %>% 
                                 select(county_name, co2e) %>% 
                                 mutate(source =  "MC_model" ),
                               nrel_predict_nonres %>% 
                                 select(county_name = county, co2e = emissions_metric_tons_co2e) %>% 
...
mn_mwh_sector <- rbind(left_join(mn_mwh,nrel %>% 
                                   filter(year == 2021,
                                          source == "Electricity"), 
                                 by = "county") %>% 
                         mutate(mwh = mwh * residential,
                                sector = "Residential") %>% 
                       ...
                       left_join(mn_mwh,nrel %>% 
                                   filter(year == 2021,
                                          source == "Electricity"), 
                                 by = "county") %>% 
                         mutate(mwh = mwh * (commercial + industrial),
                                sector = "Commercial/Industrial") %>% 
                         select(county, mwh, sector, source = source.x))

ctu_mwh_predict <- rbind(ctu_res_predict %>% st_drop_geometry() %>% 
                           select(ctu_name = ctu_name.x,
                                  mwh_predicted,
                                  county_name) %>% 
