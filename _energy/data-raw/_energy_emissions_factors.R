if (exists("eGRID_MROW_emissionsFactor_CO2") == FALSE) {
  ...
  mrow_factors <- epa_ghg_factor_hub$egrid %>% 
    filter(`eGrid Subregion` == "MROW (MRO West)",
  ...
  eGRID_MROW_emissionsFactor_CO2 <- mrow_factors %>% filter(emission == "lb CO2") %>% magrittr::extract2("value")
  eGRID_MROW_emissionsFactor_CH4 <- mrow_factors %>% filter(emission == "lb CH4") %>% magrittr::extract2("value")
  eGRID_MROW_emissionsFactor_N2O <- mrow_factors %>% filter(emission == "lb N2O") %>% magrittr::extract2("value")
