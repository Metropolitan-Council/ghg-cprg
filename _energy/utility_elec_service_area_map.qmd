...
WI_inScopeUtilities_fullServTerr <- read_rds(here(
...
To identify the utilities operating within our 11-county study area, we utilized data sets published by state sources. The outcome of our data collection yields a list of distinct utilities supplying activity data and resides within [distinct_natGas_util_WI.RDS](data/distinct_natGas_util_WI.RDS) (4 utilities, all investor-owned) and [distinct_natGas_util_type_MN.RDS](data/distinct_natGas_util_type_MN.RDS) (7 utilities, six investor-owned and one municipally-owned).  
...
# Create a color palette
utilities <- unique(WI_inScopeUtilities_fullServTerr$utility_name)
color_palette <- colorFactor(palette = "viridis", domain = utilities)
...
fig_wisconsin_elec_servTerrs <- council_leaflet() %>%
  addPolygons(
    data = cprg_county %>%
      sf::st_transform(4326),
    fill = FALSE,
  ...
  addPolygons(
    data = WI_inScopeUtilities_fullServTerr %>% sf::st_transform(4326),
    fill = TRUE,
    fillColor = ~ color_palette(utility_name),
    stroke = TRUE,
    color = "gray",
    weight = 2,
    popup = WI_inScopeUtilities_fullServTerr$utility_name
  ) %>%
  fitBounds(
    lng1 = bound_box[[1]],
    lng2 = bound_box[[3]],
