...
Note for solid waste, the global warming potentials used to calculate CO~2~e are sourced AR4, not AR5, like the rest of our inventory. However, when we dug into documentation for the EPA WARM tool [@usepaWARMManagementPractices2019], we found that the differences in total CO~2~e emissions were negligible. 
...
epa_ghg_factor_hub$egrid %>% 
  ...
  mutate(GHG = case_when(GHG == "CO2"~ "CO₂",
                         GHG == "CH4" ~ "CH₄",
                         GHG == "N2O" ~ "N₂O",
                         GHG == "cf4" ~ "CF₄",
...
epa_ghg_factor_hub$stationary_combustion %>% 
  filter(per_unit %in% c("scf",
                         "mmBtu"),
         emission != "mmBtu") %>% 
  tidyr::separate_wider_delim(emission, delim = " ",
                  names = c("Quantity",
                            "GHG")) %>% 
  mutate(Quantity = ifelse(Quantity == "g", "Grams", "Kilograms"),
         GHG = stringr::str_trim(GHG),
         GHG = 
           case_when(GHG == "CO2"~ "CO₂",
                     GHG == "CH4" ~ "CH₄",
                     GHG == "N2O" ~ "N₂O",
                     GHG == "cf4" ~ "CF₄",
                     GHG == "co2_co2_equivalent" ~ "CO₂e",
...
```
```{r tbl-moves-factors}
#| tbl-cap: "Transportation emissions factors. Source: EPA MOVES4" 
#| out-width: "95%"
epa_moves %>%
  ...
  mutate(
    ...
    GHG = case_when(GHG == "co2"~ "CO₂",
                    GHG == "ch4" ~ "CH₄",
                    GHG == "n2o" ~ "N₂O",
                    GHG == "cf4" ~ "CF₄",
                    GHG == "co2_co2_equivalent" ~ "CO₂e",
                    GHG == "HFC-152a" ~ GHG)) %>% 
      select(`Vehicle weight` = vehicle_weight,
             GHG,
