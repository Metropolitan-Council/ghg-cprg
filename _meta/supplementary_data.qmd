# Supplementary data {#sec-supplementary-data}
```{r  include=FALSE}
...
### Transportation emissions rates 

We can calculate gross emissions rates in grams emitted per vehicle mile traveled for each county by combining the historical VMT (@sec-state-dot-data), VMT forecasts, (@sec-regional-travel-demand-model), compiled historical annual emissions (@sec-epa-emissions-data-transportation), and emissions forecasts (@sec-emissions-forecast-with-moves). 

These emissions rates are comprehensive, in that they do not account for differences in vehicle or fuel type. 

Compilation provided in [calc_gross_emissions_rates.R](../_transportation/data-raw/calc_gross_emissions_rates.R). 

```{r fig-gross-emissions-rates-historical}
#| fig-cap: "Gross emissions rates, 2010-2022"
#| out-width: "95%"

fig_gross_emissions_rates_historical <- gross_emissions_rates %>%
  filter(
    vmt_data_source != "RTDM"
  ) %>%
  plot_ly(
    x = ~vmt_year,
    y = ~emissions_per_mile,
    color = ~county_name,
    colors = "Dark2",
    type = "scatter",
    mode = "lines+markers",
    ...
    hoverinfo = "text",
    hovertext = ~ paste0(
      county_name, " County, ", vmt_year, " ", "<br>",
      round(emissions_per_mile), " Grams CO<sub>2</sub>e per vehicle mile traveled"
    )
  ) %>%
  plotly_layout(
    "Gross transportation emissions rates",
    y_title = "Grams CO<sub>2</sub>e per vehicle mile traveled",
    x_title = "Year",
    subtitle = "Compiled EPA emissions for all vehicle types, DOT annual vehicle miles traveled",
    legend_title = "County"
  ) %>%
  layout(legend = list(
    orientation = "h",
...
fig_gross_emissions_rates_historical
```
...
```{r tbl-emissions-rates-forecast}
#| tbl-cap: "Forecasted annual gross emissions rates by county, 2050"
#| out-width: "95%"

tbl_emissions_rates_forecast <- gross_emissions_rates %>%
  ungroup() %>%
  filter(
    vmt_data_source == "RTDM",
    vmt_year == 2050
  ...
  select(
    `County` = county_name,
    `VMT data source` = vmt_data_source,
    `Emissions data source` = emissions_data_source,
    `Grams CO₂e per vehicle mile traveled` = emissions_per_mile
  ) %>%
  gt() %>%
  gt::opt_row_striping() %>%
  tab_style(
