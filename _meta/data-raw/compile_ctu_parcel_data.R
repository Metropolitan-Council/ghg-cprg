...
mn_parcel_map <- mn_parcel_predict %>%
  ...
  mutate(ctu_id = case_when(
    CTU_NAME == "Credit River" ~ # incorporated as city in 2021
      cprg_ctu$gnis[cprg_ctu$ctu_name == "Credit River"],
    CTU_NAME == "Empire Township" ~ # incorporated as city in 2024
      cprg_ctu$gnis[cprg_ctu$ctu_name == "Empire"],
    TRUE ~ as.numeric(CTU_ID_TXT)
  )) %>%
  left_join(cprg_ctu, by = c(
    "ctu_id" = "gnis",
    "CO_NAME" = "county_name"
  )) %>%
  st_as_sf()
...
ggplot(
  mn_parcel_map %>%
    filter(
      mc_classification == "single_family_home",
      CO_NAME != "Hennepin"
    ),
...
# missing CTUs, tbd if this can be used
#
