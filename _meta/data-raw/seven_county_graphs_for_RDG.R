...
msa_inv <- read_rds("_meta/data/cprg_county_emissions.RDS") %>% 
  ...
  mutate(sector_use = if_else(sector == "Energy", "Building Fuel",sector),
...
# sector widegraph
sector_graph <- ggplot(msa_total_inv %>% group_by(sector_use) %>%  summarize(emissions = sum(emissions)),
                       aes(x = sector_use, y = emissions / 1000000, fill = sector_use)) + geom_bar(stat = "identity") +
...
subsector_graph <- ggplot(msa_total_inv,
                       aes(x = sector_use, y = emissions / 1000000, fill = source_use)) + geom_bar(stat = "identity") +
...
county_elec <- ggplot(msa_inv %>% filter(sector_use == 'Electricity'),
...
county_transport <- ggplot(msa_inv %>% filter(sector_use == 'Transportation'),
                          aes(x = geog_name  , y = emissions_metric_tons_co2e / 1000000, fill = source_use)) + geom_bar(stat = "identity") +
  scale_fill_manual(values = color_palette_vector) + theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
...
county_waste <- ggplot(msa_inv %>% filter(sector_use == 'Waste'),
                           aes(x = geog_name  , y = emissions_metric_tons_co2e / 1000000, fill = source_use)) + geom_bar(stat = "identity") +
  scale_fill_manual(values = color_palette_vector) + theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
...
county_seq <- ggplot(msa_inv %>% filter(sector_use == 'Nature'),
                       aes(x = geog_name  , y = emissions_metric_tons_co2e / 1000000, fill = source_use)) + geom_bar(stat = "identity") +
  scale_fill_manual(values = color_palette_vector) + theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
