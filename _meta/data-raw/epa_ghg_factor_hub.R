...
stationary_combustion <- bind_rows(
  ...
  tidyr::separate_wider_delim(fuel_factor,
                              ...
                              names = c("emission",
                                        "per_unit")) %>% 
  mutate(emission = stringr::str_trim(emission),
...
onroad_gas <- bind_rows(passenger_gas,
                        light_duty_gas,
                        heavy_duty_gas,
                        moto_gas)
...
scope3_cat4_transportation <- readxl::read_xlsx("_meta/data-raw/ghg-emission-factors-hub-2021.xlsx",
                                                ...
                                                col_names = c(
                                                  ...
                                                  "g N2O per unit",
                                                  "Units")) %>% 
  mutate(vehicle_type = c("Medium and Heavy-Duty Truck",
                          "Passenger Car",
                          "Light-Duty Truck",
