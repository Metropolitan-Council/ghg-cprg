...
sw_scenarios <- gcam %>% 
  filter(subsector_mc == "Solid waste",
         scenario %in% c("PPP after Fed RB",
                         "CP after Fed RB")
  )


sw_gcam_targets <- sw_scenarios %>% 
...
sw_emissions_bau <- total_pop %>% 
  filter(inventory_year >= 2022) %>% 
  left_join(emissions_per_capita %>% 
              select(county_name, emissions_per_capita),
            by = "county_name") %>% 
  mutate(value_emissions = value * emissions_per_capita,
         scenario = "bau") %>% 
...
bau_2025 <- sw_emissions_pathways %>%
...
diverging_data <- sw_emissions_pathways %>%
  filter(emissions_year >= 2022) %>%
...
emissions_gg <- ggplot() +
  ...
  geom_point(
    data = data.frame(emissions_year = 2050, value_emissions = sw_target),
    aes(x = emissions_year, y = value_emissions),
    shape = "*",    # asterisk
...
sw_2030 <- sw_emissions_proj %>% 
  filter(emissions_year == 2030) 

bau2030 <- sw_2030 %>% filter(scenario == "bau") %>% pull(value_emissions)

ppp2030 <- sw_2030 %>% filter(scenario == "ppp") %>% pull(value_emissions) -
  bau2030

nz2030 <- sw_2030 %>% filter(scenario == "nz") %>% pull(value_emissions) -
  bau2030
...
sw_2050 <- sw_emissions_proj %>% 
  filter(emissions_year == 2050) 

bau2050 <- sw_2050 %>% filter(scenario == "bau") %>% pull(value_emissions)

ppp2050 <- sw_2050 %>% filter(scenario == "ppp") %>% pull(value_emissions) -
  bau2050

nz2050 <- sw_target - bau2050
