...
demand_plot <- ggplot(onet_scored_OID, aes(x = jvr_formula, 
                                    y = projected_growth_rate, 
                                    size = emp_count,
                                    color = demand_stars_fct
)) +
...
make_demo_block <- function(df, soc) {
  ...
  sex  <- demo %>% filter(demo_var == "sex_label")  %>% select(Sex = group, Sex_pct = pct)
  age  <- demo %>% filter(demo_var == "age_group")  %>% select(Age = group, Age_pct = pct)
  edu  <- demo %>% filter(demo_var == "educ_group") %>% select(Edu = group, Edu_pct = pct)
  race <- demo %>% filter(demo_var == "race_label") %>% select(Race = group, Race_pct = pct)
...
# Occ group titles
maj_occs <- soc_2018_df %>% filter(soc_code %in% c("110000","130000","150000","170000",
                         "470000","490000","510000","530000")) %>% 
...
jobstat <- read_csv(file.path(occ_data_path, "jobstat_skillsmatch.csv")) %>% 
  clean_names() %>% 
  mutate(code_x = as.character(code_x),
...
cip_6digit <- cip_soc %>% 
  ...
  mutate(cip_2digit = paste0(str_sub(cip2020code, 1, 2), ".0000"),
...
plot <-  ggraph(graph, layout = layout) +
  ...
  scale_edge_colour_manual(
    ...
    labels = c("field" ="Shared field of study (CIP-4)",
...
plot <-  ggraph(graph, layout = layout) +
  ...
  geom_edge_link(
    aes(
      edge_width = log(shared_cert+1)), edge_colour = "purple", edge_alpha = .5
  )+
  scale_edge_width(range = c(0.5, 2), guide = "none") +
...
cip_soc <- read_xlsx(file.path(gen_data_path,"CIP2020_SOC2018_Crosswalk.xlsx"), sheet = "CIP-SOC") %>% 
  mutate(soc_code = str_remove(SOC2018Code, "-"),
...
occ_geodata_underBA <- geodata_underBA %>% 
  ...
  left_join(cip_soc %>% select(-cip_4), by = c("cip" = "cip_2"),
            suffix = c("_4", "_2")) %>% 
  mutate(soc_code = coalesce(soc_code_4, soc_code_2),
...
edu_programs_priorityoccs_underBA <- occ_geodata_underBA %>% 
  ...
  distinct() %>% 
  left_join(occ_geodata_underBA %>% filter(soc_code %in% all_occ_underBA$soc_code) %>% 
              select(cip, soc_code, soc_title),
            by = "cip", suffix = c("", "_related")) %>% 
  filter(soc_code != soc_code_related) %>% 
    group_by(instructional_program) %>%
  summarize(list_priorityocc = paste(sort(unique(soc_title)), collapse = "\n"),
...
underBA_program_outcomes <- occ_geodata_underBA %>% 
  ...
  select(graduation_year, school_location, instructional_program, award, graduates, graduates_employed_in_mn_during_the_2nd_year,
  ...
  distinct()  %>% 
  mutate(across(
    c(graduates_employed_in_mn_during_the_2nd_year,
  ...
  group_by(award) %>% 
  summarise(grads = sum(graduates), grads_employed_2yrsout = sum(graduates_employed_in_mn_during_the_2nd_year, na.rm = TRUE),
...
edu_programs_priorityoccs_overBA <- occ_geodata_overBA %>% 
  ...
  select(soc_code, soc_title, cip, instructional_program) %>% 
  distinct() %>% 
  left_join(occ_geodata_underBA %>% filter(soc_code %in% all_occ_overBA$soc_code) %>% 
              select(cip, soc_code, soc_title),
            by = "cip", suffix = c("", "_related")) %>% 
  filter(soc_code != soc_code_related) %>% 
  group_by(instructional_program) %>%
