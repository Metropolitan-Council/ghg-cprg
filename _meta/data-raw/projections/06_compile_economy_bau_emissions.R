...
aviation_mpca <- read_rds("_meta/data/gcam/mpca_subsector_gcam.RDS") %>%
  filter(
    subsector_mc == "Aviation", scenario == "CP after Fed RB",
    emissions_year >= 2025
  ) %>%
...
print(bau_projection_plot)

message("Saving economy-wide BAU projections plot to: \n\t ~/imgs/eleven_county_bau_projections.png")
ggsave(
  plot = bau_projection_plot,
  filename = paste0(here::here(), "/imgs/eleven_county_bau_projections.png"), # add your file path here
  width = 12,
  height = 6,
  units = "in",
  dpi = 300,
...
message("Finished economy-wide BAU projections")
