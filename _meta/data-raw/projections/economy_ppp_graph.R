...
nonres_elec_bau <- bind_rows(nonres_elec_bau,
                             nonres_elec_bau %>% 
                               filter(emissions_year >= 2025) %>% 
                               mutate(scenario = "ppp"))

aviation_mpca <- read_rds("_meta/data/gcam/mpca_subsector_gcam.RDS") %>% 
  filter(subsector_mc == "Aviation", scenario %in% c("CP after Fed RB",
                                                     "PPP after Fed RB"),
         emissions_year >= 2025) %>% 
...
ggsave(plot = ppp_projection_plot,
       filename = paste0(wd,"/eleven_county_ppp.png"),  # add your file path here
       width = 12,          
       height = 6,          
       units = "in",
       dpi = 300, 
