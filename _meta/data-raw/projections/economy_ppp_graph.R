...
tr_ppp <- readRDS("./_meta/data-raw/projections/tr_pathways.rds") %>% 
  filter(scenario == "ppp")
sw_ppp <- readRDS("./_meta/data-raw/projections/waste_pathways.rds")%>% 
  filter(scenario == "ppp")
res_ppp <- readRDS("_meta/data/residential_pathways.RDS") %>% 
  filter(scenario == "ppp")
nonres_ng_ppp <- readRDS("./_meta/data-raw/projections/nonres_ng_pathways.rds")%>% 
  filter(scenario == "ppp")
# nonres_elec_ppp <- readRDS("./_meta/data-raw/projections/nonres_elec_bau.rds")
ind_ppp <- readRDS("./_meta/data-raw/projections/ind_pathways.rds")%>% 
  filter(scenario == "ppp")
ag_ppp <- readRDS("./_meta/data-raw/projections/ag_pathways.rds") %>% 
  filter(scenario == "ppp")
ns_ppp <- readRDS("_meta/data/regional_ns_forecast.RDS") %>% 
  filter(scenario == "ppp")



ppp <- bind_rows(sw_ppp %>% 
                   mutate(sector = "Waste"),
                 tr_ppp %>% 
                   mutate(sector = "Transportation"),
                 aviation_bau_interpolated %>%
...
ggplot() +
  ...
  geom_area(data = base_data_pos, 
            aes(x = emissions_year, y = value_emissions, fill = sector),
            position = "stack") +
  # Add negative emissions (natural systems) below zero
  geom_area(data = base_data_neg,
            aes(x = emissions_year, y = value_emissions, fill = sector)) +
  
  geom_line(data = base_data_tot,
            aes(x = emissions_year, y = total_emissions, color = "Business as usual"),
