...
ind_mpca <- read_rds("_meta/data/gcam/mpca_subsector_gcam.RDS") %>%
  filter(
    sector == "Industry", scenario == "Current policies",
    emissions_year >= 2025
  ) %>%
...
aviation_mpca <- read_rds("_meta/data/gcam/mpca_subsector_gcam.RDS") %>%
  filter(
    subsector_mc == "Aviation", scenario == "Current policies",
    emissions_year >= 2025
  ) %>%
  select(emissions_year, subsector_mc, proportion_of_2020)
...
ind_bau <- county_emissions %>%
  filter(
    category %in% unique(ind_mpca$subsector_mc),
    emissions_year == 2020,
    !county_name %in% c(
      "St. Croix",
      "Pierce",
  ...
  mutate(
    value_emissions = value_emissions_2020 * proportion_of_2020,
...
comm_bau_out <- comm_bau %>%
  ...
  left_join(res_bau %>%
    mutate(
      mwh_ef = electricity_emissions / mwh,
      mcf_ef = natgas_emissions / mcf
    ) %>%
    select(inventory_year, mwh_ef, mcf_ef)) %>%
  mutate(
    electricity_emissions = mwh * mwh_ef,
