...
county_emissions <- readRDS(file.path(here::here(), "_meta/data/cprg_county_emissions.RDS")) %>% 
  filter(!county_name %in% c("St. Croix",
                           "Pierce",
                           "Chisago",
                           "Sherburne"))
...
county_emissions %>% filter(emissions_year == year_use,
                            sector == "Residential",
...
county_emissions %>% filter(emissions_year == year_use,
  ...
  group_by(source) %>% summarize(value_emissions = sum(value_emissions))
...
county_emissions %>% filter(emissions_year == 2005,
                            value_emissions > 0) %>% 
  pull(value_emissions) %>% sum()
