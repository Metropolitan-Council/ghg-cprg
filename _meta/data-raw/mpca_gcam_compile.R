...
waste <- c(
  "MMSW landfills", "MMSW composting", "MMSW compost preprocessing",
  "Yard waste composting", "Yard waste preprocessing"
)

state_projections %>%
  filter(sector == "LULUCF", values_emissions < 0) %>%
  distinct(source) %>%
  print(n = 50)
...
scenarios_sources_annual <- state_projections %>%
  ...
  mutate(
    value_2005 = values_emissions[emissions_year == 2005],
    value_2020 = values_emissions[emissions_year == 2020]
  ) %>%
  # Calculate the proportion
  mutate(
    proportion_of_2005 = values_emissions / value_2005,
