...
comm_11 <- dplyr::filter(comm, 
                  (County.Name %in% c('Hennepin County','Ramsey County','Anoka County','Dakota County','Carver County',
...
our_tracts <- dplyr::rows_append(mn_tracts,wi_tracts) %>% 
  dplyr::left_join(.,comm_11 %>% dplyr::select(Census.tract.2010.ID,Identified.as.disadvantaged),
...
lidac_out <- comm_lidac %>% dplyr::select(Census.tract.2010.ID, County.Name, Identified.as.disadvantaged)
