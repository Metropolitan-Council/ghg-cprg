...
economy_projections <- county_emissions %>%
  filter(year == 2005, sector != "Nature",
  ...
  mutate(emissions_metric_tons_co2e = proportion_of_2005 * emissions_metric_tons_co2e) %>% 
...
economy_inventory_projections <- bind_rows(
 ...
 group_by(sector) %>% 
 mutate(emissions_metric_tons_co2e = imputeTS::na_kalman(emissions_metric_tons_co2e))
...
fig_economy_current_policies <- plot_ly(
  ...
  data = economy_inventory_projections %>% 
    filter(scenario %in% c("Inventory", "Current policies"),
           sector != "Nature"),
  x = ~emissions_year,
...
fig_economy_net_zero <- plot_ly(
  ...
  data = economy_inventory_projections %>% 
    filter(scenario %in% c("Inventory", "Net-zero pathway"),
           sector != "Nature"),
  x = ~emissions_year,
  y = ~emissions_metric_tons_co2e,
  color = ~sector, #
