testthat::test_that("County data is as expected", {
  ...
  testthat::expect_equal(cprg_county$NAME,
                         c("Scott", "Chisago", "Dakota", "Anoka", "Ramsey", "Washington", 
                           "Sherburne", "Hennepin", "Carver", "Pierce", "St. Croix"))
  testthat::expect_equal(names(cprg_county),c("STATE", "STATEFP", "COUNTYFP", "GEOID",
                                              "NAME", "NAMELSAD", 
                                              "geometry"))
...
testthat::test_that("CTU data is as expected", {
  ...
  cprg_ctu <- readRDS(file.path(here::here(), "R/data/cprg_ctu.RDS"))
  
  testthat::expect_equal(nrow(cprg_ctu), 289)
  testthat::expect_equal(names(cprg_ctu), c("CTU_NAME", "CTU_CLASS", "COUNTY_NAM",
                                            "STATEFP", "STATE", 
