testthat::test_that("Compiled onroad emissions", {
  ...
  testthat::expect_equal(
    ...
    c("emissions_metric_tons_co2e", "co2", "ch4", "n2o", "co", "no", 
      "nox", "so2", "nh3", "pm10_pri", "pm25_pri")
  )
  
  testthat::expect_equal(
    unique(epa_onroad_emissions_compile$vehicle_type),
    c("Motorcycles", "Passenger cars", "Passenger trucks", "Light commercial trucks", 
  ...
  testthat::expect_equal(
    unique(epa_onroad_emissions_compile$fuel_type),
    c("Gasoline", "Diesel", "Compressed natural gas (CNG)", "Ethanol (E-85)", 
      "Electric")
  )
