...
sum(park_carbon$sequestration_potential) / sum(county_seq_7$emissions_metric_tons_co2e) # 5.56%
sum(park_carbon$stock_potential) / sum(county_seq_7$stock_potential) # 7.19%


park_carbon_agg <- park_carbon %>% filter(!is.na(park)) %>%  group_by(county) %>% 
  summarize(area = sum(area),sequestration_potential = sum(sequestration_potential), stock_potential = sum(stock_potential)) %>% 
  mutate(seq_per_area = sequestration_potential/area, stock_area = stock_potential/area)

park_county_comp <- bind_rows(bind_rows(pivot_longer(park_carbon_agg %>% select(county,seq_area, stock_area),
                                 cols = c(seq_area, stock_area)) %>% mutate(geography = "Regional Parks"),
...
#### graph comparing sequestration rates per area in parks vs counties
