...
mn_wetlands <- terra::rast("./_nature/data-raw/mn_wetlands/mn_restorable_wetland_index.tif")
crs_tmp <- terra::crs(mn_wetlands)


# load cprg county and ctu boundaries
cprg_county <- readRDS("_meta/data/cprg_county.RDS")
cprg_ctu <- readRDS("_meta/data/cprg_ctu.RDS") %>% mutate(idx = 1:nrow(.))
...
# mn_wetlands_crop <- mn_wetlands %>% terra::crop(cprg_county_tmp)
# mn_wetlands_actual <- mn_wetlands_crop %>% terra::project(crs_use, method = "near")
# 
# 
# # remove non-county pixels
# cprg_wetlands <- terra::mask(mn_wetlands_actual, cprg_county)
...
inpath_wetlands_high_priority <- paste0(here::here(), "/_nature/data-raw/restorable_wetlands_gdb/RestorableWetlands_CCAP.gdb")

wetlands_high_priority <- st_read(inpath_wetlands_high_priority, layer = "RestorableWetlands_CCAP")

# sum down total acreage in the wetlands_high_priority layer
wetlands_summary <- wetlands_high_priority %>%
  st_drop_geometry() %>%
  # group_by(Wetland_Type) %>%
  summarize(
    total_acreage = sum(ACRES, na.rm = TRUE),
    total_km2 = total_acreage * 0.00404686,
    count_polygons = n()
  )
...
area_values <- mask(cellSize(cprg_wetlands, unit = "km"), cprg_county)
# terra::plot(area_values)
...
nlcd_county <- data.frame(
  ...
  secondary_cover_type = as.character(),
  area = as.numeric(),
...
lapply(start_year:end_year, function(year) {
  ...
  # area_values <- terra::extract(area_values, cprg_county)
  
  pctBar(50, "Extracting wetland values (this can take awhile)")
  restWetlands_values <- terra::extract(cprg_wetlands, cprg_county)
  ...
  if (tcc_available) {
    ...
    lc_df <- as_tibble(data.frame(
      ...
      wetland_cover = restWetlands_values[, 2],
...
nlcd_lc <- terra::rast(nlcd_lc_files[get_year(nlcd_lc_files) %in% yr])
