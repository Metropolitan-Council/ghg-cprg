...
wetlands_raster_cprg <- readRDS("_nature/data/mn_wetlands_msa.rds")

# If export_plots_path doesn't exist, create it
if (!dir.exists("_nature/data-raw/nlcd_land_cover_by_year/")) {
  dir.create("_nature/data-raw/nlcd_land_cover_by_year/", recursive = TRUE)
}
...
lapply(start_year:end_year, function(year) {
  ...
  if (tcc_available) {
    ...
    lc_df <- as_tibble(data.frame(
      ...
      wetlands_restorable = wetlands_values[, 2],
    ...
    # save this file to "_nature/data-raw/nlcd_land_cover_by_year/"
    saveRDS(
      lc_df,
      paste0(
        "_nature/data-raw/nlcd_land_cover_by_year/nlcd_land_cover_by_pixel_",
        year,
        ".RDS"
      )
    )
