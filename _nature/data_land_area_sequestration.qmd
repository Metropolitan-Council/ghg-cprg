...
tbl_stock <- kable(
  land_cover_carbon %>%
    dplyr::select(
      land_cover_type,
      `Metric tons CO₂e per square kilometer` = seq_mtco2e_sqkm
    ) %>%
    left_join(., seq_citations) %>%
    unite("Land Cover Type", c("land_cover_type", "Citation"), sep = ""),
  row.names = FALSE, format = "markdown"
