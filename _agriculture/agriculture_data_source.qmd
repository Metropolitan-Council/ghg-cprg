...
Agricultural emissions are derived from two primary sources. First is the USDA agricultural census [@usdaUSDANationalAgricultural] which provides county level livestock head counts, crop production, and fertilizer sales. Second is the EPA State Inventory Tool (SIT) [@epaUSERGUIDEESTIMATING2024] that provides state specific values for emission factors as well as activity data such as state-wide fertilizer application (different from sales). These are both the highest rank of data.
