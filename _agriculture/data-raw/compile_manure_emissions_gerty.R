...
manure_ch4 <- left_join(livestock,
  ...
  ungroup() %>% 
  left_join(animal_manure_complete,
            join_by(year, livestock_type)) %>% 
  mutate(mt_co2e_by_state = mt_co2e * percentage)
...
liquids <- ag_manure_mgmt_complete %>%
  ...
  summarize(liquids_perc = sum(percentage))

solids <- ag_manure_mgmt_complete %>%
  filter(mgmt_system %in% c(
    "Solid Storage",
    "Dry Lot",
  ...
  mutate(state = if_else(state == "MN", "Minnesota", "Wisconsin")) %>%
  group_by(year, state, livestock_type) %>%
  summarize(solids_perc = sum(percentage))
...
manure_n2o <- left_join(
  ...
  mutate(
    ...
    mt_n2o_solids = head_count *
      kg_nex_head_yr * # to kg_nex_yr
      (1 - ag_constants_vec["VolPercent"]) * # % unvolatized
      solids_perc * # stored as solids
      ag_constants_vec["SolidEF"] * # N2O-N2 emissions
      ag_constants_vec["N2O_N2"] / # N2O emissions (kg)
      1000, # to mt
    mt_co2e_liquids = mt_n2o_liquids * gwp$n2o,
    mt_co2e_solids = mt_n2o_solids * gwp$n2o
  ) %>%  pivot_longer(
    cols = c(mt_co2e_liquids, mt_co2e_solids,
             mt_n2o_liquids, mt_n2o_solids),
...
# soil nitrogen emissions from eventual application (I think)
...
manure_emissions <- bind_rows(
  manure_ch4 %>% group_by(year, county_name, storage_state) %>%
    summarize(mt_co2e_by_state = sum(mt_co2e_by_state), mt_gas  = sum(mt_ch4 * percentage)) %>%
    mutate(category = "livestock", source = "manure_management", gas_type = "ch4"),
  manure_n2o %>% group_by(year, county_name, storage_state) %>%
    summarize(mt_co2e_by_state = sum(mt_co2e), mt_gas = sum(mt_n2o)) %>%
    mutate(category = "livestock", source = "manure_management", gas_type = "n2o"),
  manure_soils_emissions %>% group_by(year, county_name) %>%
    summarize(
      mt_co2e_by_state = sum(MT_co2e_manure_application + MT_co2e_pasture),
  ...
  nex_runoff_emissions %>%
    group_by(year, county_name, storage_state) %>% 
    summarize(
      mt_co2e_by_state = sum(mt_co2e_by_state),
      mt_gas = sum(mt_n2o * percentage)
    ) %>%
