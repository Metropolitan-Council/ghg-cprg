...
emissions_total <- county_emissions_summary_2022 %>%
  filter(category != "Sequestration") %>%
  pull(value_emissions) %>%
  sum(na.rm = T)

seq_total <- county_emissions_summary_2022 %>%
  filter(category == "Sequestration") %>%
  pull(value_emissions) %>%
  sum() * -1
...
electricity_2022 <- county_emissions_summary_2022 %>%
...
sector_order <- county_emissions %>%
  filter(emissions_year >= 2005 & emissions_year <= 2022, category != "Sequestration") %>%
  group_by(sector) %>%
...
sector_order <- c("Natural Systems", sector_order)

category_order <- county_emissions %>%
  select(category, sector) %>%
  unique() %>%
  mutate(sector = factor(sector, levels = sector_order, ordered = TRUE)) %>%
  pull(category)


county_emissions_2022 <- county_emissions %>%
  filter(emissions_year == 2022) %>%
...
county_emissions_2022$sector <- factor(county_emissions_2022$sector,
  levels = rev(sector_order),
  ordered = TRUE
) %>%
  droplevels()


county_emissions_2022$category <- factor(county_emissions_2022$category,
  levels = rev(names(category_colors) %>% unique())
)


fig_county_emissions_sector <- plot_ly(
  data = county_emissions_2022 %>%
  ...
  plotly_layout("11-county regional greenhouse gas emissions, 2022",
    subtitle = "",
    x_title = "Sector",
...
Emissions in the region fell by `r scales::percent((emissions_2005 - emissions_total) / emissions_2005, accuracy = 0.1)` from 2005 to 2022. This is principally attributed to decarbonization of the electric grid, which fell `r scales::percent((electricity_2005 - electricity_2022) / electricity_2005, accuracy = 0.1)` from 2005 to 2022. Other sectors showed small increases or decreases.
...
emissions_data <- county_emissions %>%
  filter(
    emissions_year >= 2005 & emissions_year <= 2022,
    category != "Sequestration"
  ) %>%
...
p <- plot_ly(source = "fig-temporal-emissions")
...
emissions_through_time <- p %>%
  add_trace(
    ...
    type = "scatter",
    mode = "none",
    fill = "tozeroy", # Prevent stacking
    color = ~sector,
    colors = c(unlist(sector_colors), Freshwater = "#1E90FF"),
    hoverinfo = "text",
    text = ~ paste0(
  ...
  plotly_layout("11-county regional greenhouse gas emissions: 2005-2022",
    subtitle = "",
    x_title = "Year",
    y_title = "Metric tons CO<sub>2</sub>e",
    legend_title = "Sector"
...
#| fig-cap: "County emissions by sector in 2022"
#| out-width: "95%"
...
county_emissions_2022 <- county_emissions %>%
  ...
  mutate(
    # sector = ifelse(category == "Freshwater", "Natural Systems", sector),
    sector = factor(sector, levels = rev(sector_order), ordered = TRUE)
  )
...
county_emissions_raw <- plot_ly(
  ...
  plotly_layout("County greenhouse gas emissions, 2022",
    subtitle = "",
    x_title = "County",
    y_title = "Metric tons CO<sub>2</sub>e",
    legend_title = "Sector",
...
#| fig-cap: "County emissions per capita by sector in 2022"
#| out-width: "95%"

county_emissions_2022_per_capita <-
  county_emissions_2022 %>%
  mutate(sector = factor(sector, levels = rev(sector_order), ordered = TRUE)) %>%
  plot_ly(
    ...
    hovertemplate = ~ paste0(
      county_name, " County", "<br>",
      sector, " - ", category, ", ", source, "<br>",
      round(emissions_per_capita, digits = 2), " million metric tons CO<sub>2</sub>e per person", "<br>",
      "<extra></extra>"
    )
  ) %>%
  plotly_layout("County greenhouse gas emissions per capita, 2022",
    subtitle = "",
    x_title = "County",
    y_title = "Metric tons CO<sub>2</sub>e per person",
    legend_title = "Sector",
    barmode = "stack"
  ) %>%
  layout(legend = list(traceorder = "reversed"))
